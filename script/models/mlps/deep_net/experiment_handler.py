...
import models.mlps.deep_net.model as model
...
class Model_Handler(object):
  def __init__(self, 
    ...
    self.net = model.Deep_Net(nb_features, nb_layers, nb_hidden)
    self.lrate = lrate
  ...
  def fit(self, train_X, train_y, test_X, test_y):
    ...
    logging.warning("Training net")
    for i in range(self.nb_epoch):
      logging.info("Epoch {} lrate: {}".format(i+1, self.lrate))
      epoch_loss = self._train_one_epoch(train_X, train_y, criterion, optimizer)
      logging.info("Epoch loss: {0:.4e}\n".format(epoch_loss))
  ...
  def _train_one_epoch(self, X, y, criterion, optimizer):
    ...
    for i in range(nb_batch):
      ...
      batch_X = Variable(torch.Tensor(X[batch_idx]))
